...
def mobile(request , data=None):
    if data == None:
        ...
        mobiles = Product.objects.filter(category = 'M').filter(brand =data)#filter under filter operations
    elif data == 'below':
        mobiles = Product.objects.filter(category = 'M').filter(discounted_price__lt=10000)#lesser than
    elif data == 'above':
        mobiles = Product.objects.filter(category = 'M').filter(discounted_price__gt=10000)#greater than
